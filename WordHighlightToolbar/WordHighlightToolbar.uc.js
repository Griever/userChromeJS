...
//		},
	...
	addToolbar: function() {
		...
		reloadbutton.setAttribute("tooltiptext", "ワードをハイライトし直す");
		...
		addbutton.setAttribute("tooltiptext", "ワードを追加");
	...
	addButton: function(toolbar, aItem, aButton) {
		...
		if (!button) {
			...
			button.setAttribute('tooltiptext', [
				'クリック or ホイールダウンで次を検索',
				'Shift+クリック or ホイールアップで前を検索',
				'ホイールクリックで削除'].join('\n'));
	...
	addWord: function(aWord, aBold, aRange) {
		if (!aWord) {
			aWord = prompt('ワードを追加', getBrowserSelection());
			aBold = true;
		}
		if (!aWord) return;
